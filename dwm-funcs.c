...
{
    strncpy(
        m->ltsymbol, m->lt[m->sellt]->symbol, sizeof(m->ltsymbol) - sizeof(char)
    );
...
Monitor *createmon(void)
{
    ...
    strncpy(m->ltsymbol, layouts[0].symbol, sizeof(m->ltsymbol) - sizeof(char));
    return m;
}
...
void setlayout(Arg *arg)
{
    ...
    strncpy(
        ...
        sizeof(selmon->ltsymbol) - sizeof(char)
    );
    if (selmon->sel)
        arrange(selmon);
    else
...
void setup(void)
{
    ...
    // signal(SIGHUP, some_sighup);
    // signal(SIGTERM, some_sigterm);
