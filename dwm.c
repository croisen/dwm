...
void buttonpress(XEvent *e)
{
    ...
    if (ev->window == selmon->barwin) {
        ...
        if (i < LENGTH(tags)) {
            ...
            click = ClkLtSymbol;
        /* 2px right padding (awesomebar) + (systray)*/
        else if (ev->x >
                 selmon->ww - TEXTW(stext) + lrpad - 2 - getsystraywidth())
            click = ClkStatusText;
        else {
...
void clientmessage(XEvent *e)
{
    ...
    if (showsystray && cme->window == systray->win &&
        ...
        if (cme->data.l[1] == SYSTEM_TRAY_REQUEST_DOCK) {
            ...
            sendevent(
                c->win, netatom[Xembed], StructureNotifyMask, CurrentTime,
...
Atom getatomprop(Client *c, Atom prop)
{
     ...
     * the stored data instead of this workaround (systray) */
    Atom req = XA_ATOM;
    if (prop == xatom[XembedInfo])
        req = xatom[XembedInfo];

    if (XGetWindowProperty(
            dpy, c->win, prop, 0L, sizeof atom, False, req, &da, &di, &dl, &dl,
            &p
        ) == Success &&
        p) {
        atom = *(Atom *)p;
...
void killclient(const Arg *arg)
{
    if (!selmon->sel)
        return;
    if (!sendevent(
            selmon->sel->win, wmatom[WMDelete], NoEventMask, wmatom[WMDelete],
            CurrentTime, 0, 0, 0
        )) {
        XGrabServer(dpy);
...
void maprequest(XEvent *e)
{
    ...
    if ((i = wintosystrayicon(ev->window))) {
        sendevent(
            i->win, netatom[Xembed], StructureNotifyMask, CurrentTime,
            XEMBED_WINDOW_ACTIVATE, 0, systray->win, XEMBED_EMBEDDED_VERSION
        );
        resizebarwin(selmon);
        updatesystray();
...
void quit(const Arg *arg)
{
    ...
    // fix: reloading dwm keeps all the hidden clients hidden (awesomebar)
    Monitor *m;
    Client *c;
    for (m = mons; m; m = m->next) {
        if (m) {
...
void setfocus(Client *c)
{
    ...
    sendevent(
        c->win, wmatom[WMTakeFocus], NoEventMask, wmatom[WMTakeFocus],
        CurrentTime, 0, 0, 0
    );
}
...
void setup(void)
{
    ...
    // So sighcld was replaced by the one above?
...
void tile(Monitor *m)
{
    ...
    for (i = my = ty = 0, c = nexttiled(m->clients); c;
        ...
        if (i < m->nmaster) {
            ...
            resize(
                c, m->wx + mw + g, m->wy + ty, m->ww - mw - g - (2 * c->bw),
                h - (2 * c->bw), False
            );
            if (ty + HEIGHT(c) < m->wh)
                ty += HEIGHT(c) + gappx;
...
void unmapnotify(XEvent *e)
{
    ...
    if ((c = wintoclient(ev->window))) {
         ...
         * _not_ destroy them. We map those windows back (systray) */
...
void updatesystrayiconstate(Client *i, XPropertyEvent *ev)
{
    ...
    if (flags & XEMBED_MAPPED && !i->tags) {
        ...
        return;
    sendevent(
        i->win, xatom[Xembed], StructureNotifyMask, CurrentTime, code, 0,
...
void updatesystray(void)
{
    ...
    if (!systray) {
        ...
        XChangeWindowAttributes(
            dpy, systray->win, CWEventMask | CWOverrideRedirect | CWBackPixel,
            &wa
        );
        XMapRaised(dpy, systray->win);
        XSetSelectionOwner(
        ...
        if (XGetSelectionOwner(dpy, netatom[NetSystemTray]) == systray->win) {
            sendevent(
                root, xatom[Manager], StructureNotifyMask, CurrentTime,
